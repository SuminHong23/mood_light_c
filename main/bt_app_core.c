...
#include "driver/dac_continuous.h"  // 내부 DAC 헤더만 포함
#include "freertos/ringbuf.h"

#define RINGBUF_HIGHEST_WATER_LEVEL    (32 * 1024)//최대 수위: 이 이상 데이터가 쌓이면, 데이터를 더 이상 넣지 않고 버리거나 처리 방식 변경
#define RINGBUF_PREFETCH_WATER_LEVEL   (20 * 1024)//재생 시작 수위: 재생(I2S)을 시작하기 전에 최소한 이 정도까지 데이터를 받아야 안정적으로 재생 가능
...
//블루투스 관련 작업을 처리하는 앱 태스크로 메시지를 보내는 역할
//매개변수: msg는 작업을 설명하는 구조체 포인터(ex 오디오 시작, 정지 등)
static bool bt_app_send_msg(bt_app_msg_t *msg)
{
    //메시지가 NULL이면 아무 작업도 하지 않고 실패 처리
    if (msg == NULL) {
        return false;
    }

    //xQueueSend는 s_bt_app_task_queue라는 작업 큐에 메세지를 보냄
    //10ms 안에 큐에 넣지 못하면 실해하고 에러 로그 출력
    //__func__는 현재 함수 이름을 문자열로 반환하는 매크로
...
//Bluetooth 애플리케이션에서 메시지를 처리하는 작업 핸들러
static void bt_app_task_handler(void *arg)
{
    bt_app_msg_t msg;
...
//I2S통신을 이용하여 오디오 데이터를 출력하는 작업을 처리하는 FreeRTOS 태스크.Bluetooth A2DP Sink에서 수신한 오디오 데이터를 I2S 인터페이스로 전송
